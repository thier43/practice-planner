// run-schema.js
// Exécute schema.sql sur ta base Turso via l'API HTTP, sans passer par la CLI
// (utile sur Windows, où la CLI Turso exige maintenant WSL).
//
// Usage :
//   node run-schema.js <TURSO_DATABASE_URL> <TURSO_AUTH_TOKEN>
//
// Exemple :
...
async function main() {
  ...
  if (!url || !token) {
    console.error('Usage: node run-schema.js <TURSO_DATABASE_URL> <TURSO_AUTH_TOKEN>');
    process.exit(1);
  }
